...
└── commands/             # Click CLI commands (auto-discovered by `ai_agent_6 cmd …`)
...
The generated project ships a Click CLI exposed as `ai_agent_6` (after `make install`):

```bash
ai_agent_6 server run --reload          # dev server
ai_agent_6 db upgrade                   # apply migrations
ai_agent_6 db migrate -m "message"      # create new migration
ai_agent_6 user create-admin            # interactive admin creation
ai_agent_6 rag-ingest <path> -c docs    # ingest local files
ai_agent_6 rag-search "query" -c docs   # semantic search
ai_agent_6 rag-collections              # list collections
ai_agent_6 celery worker                # start worker
ai_agent_6 celery beat                  # start scheduler
```

Run `make help` for a categorized list, or `ai_agent_6 --help` for full CLI docs.
...
ai_agent_6 rag-ingest /path/to/docs/ --collection documents --recursive
# Pull from Google Drive (service-account auth)
ai_agent_6 rag-sync-gdrive --collection documents --folder-id <id>
# Pull from S3 / MinIO
ai_agent_6 rag-sync-s3 --collection documents --prefix docs/

# Semantic search
ai_agent_6 rag-search "your query" --collection documents
```
