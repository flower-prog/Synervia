...
```bash
uv run synervia cmd rag-ingest ./docs/ --collection docs --recursive   # ingest files/folder
uv run synervia cmd rag-search "your question" --collection docs        # semantic search
uv run synervia cmd rag-collections                                     # list collections
uv run synervia cmd rag-stats                                           # chunk/vector counts
uv run synervia cmd rag-drop <collection> --yes                         # delete a collection
```
...
```bash
uv run synervia cmd rag-sources                  # list
uv run synervia cmd rag-source-add               # add (interactive)
uv run synervia cmd rag-source-sync --all        # trigger a sync
```
