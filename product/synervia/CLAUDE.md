...
**synervia** - FastAPI application generated with [Full-Stack AI Agent Template](https://github.com/vstorm-co/full-stack-ai-agent-template).
...
# RAG
uv run synervia rag-collections
uv run synervia rag-ingest /path/to/file.pdf --collection docs
uv run synervia rag-search "query" --collection docs
uv run synervia rag-sync-s3 --collection docs

# Sync Sources
uv run synervia cmd rag-sources
uv run synervia cmd rag-source-add
uv run synervia cmd rag-source-sync
```
