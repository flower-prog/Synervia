...
Synervia connects enterprise knowledge and business systems so employees can ask questions,
analyze work, and execute governed actions from one assistant. The product is built around
three guarantees: tenant-aware knowledge access, server-side authorization for every action,
and complete approval/audit trails for sensitive operations.

The product roadmap and implementation status live in
[`../../SYNERVIA_PLAN.md`](../../SYNERVIA_PLAN.md).
...
After the one-time environment copy, day-to-day startup is just `make dev`.
...
└── commands/             # Click CLI commands (auto-discovered by `synervia cmd …`)
...
The generated project ships a Click CLI exposed as `synervia` (after `make install`):

```bash
synervia server run --reload          # dev server
synervia db upgrade                   # apply migrations
synervia db migrate -m "message"      # create new migration
synervia user create-admin            # interactive admin creation
synervia rag-ingest <path> -c docs    # ingest local files
synervia rag-search "query" -c docs   # semantic search
synervia rag-collections              # list collections
synervia celery worker                # start worker
synervia celery beat                  # start scheduler
```

Run `make help` for a categorized list, or `synervia --help` for full CLI docs.

---

## Configuration

All backend config lives in the gitignored `backend/.env`. Start from the committed
`backend/.env.example` development defaults. Key variables:
...
synervia rag-ingest /path/to/docs/ --collection documents --recursive
# Pull from S3 / MinIO
synervia rag-sync-s3 --collection documents --prefix docs/

# Semantic search
synervia rag-search "your query" --collection documents
```
